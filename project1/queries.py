...
SELECT  city AS airport_city, airlineid AS airline_id, count(*) AS inbound_flights
...
queries[5] = """
WITH    onestop_flights AS (
    SELECT  A.flightid AS flightid1, B.flightid AS flightid2, A.dest AS connectionairportid, B.local_departing_time - A.local_arrival_time AS layoverduration
    FROM    flights as A, flights as B
    WHERE   (A.dest, A.source, B.dest) = (B.source, 'JFK', 'LAX')
)
SELECT  *
FROM    onestop_flights
WHERE   layoverduration >= interval '01:00:00'
ORDER BY layoverduration;
"""
...
FROM    flight_loads
WHERE   PLF <= 0.01
ORDER BY PLF DESC, flightid ASC;
"""
...
queries[7] = """
WITH    customer_airlines AS (
    SELECT  customerid, customers.name, frequentflieron, airlineid
    FROM    customers, airlines
    GROUP BY customerid, customers.name, frequentflieron, airlineid
),      customer_flights AS (
    SELECT  customerid, airlineid, flightid
    FROM    flewon NATURAL INNER JOIN flights
),      customer_joined AS (
    SELECT  customerid, airlineid, name, frequentflieron, count(flightid)
    FROM    customer_airlines NATURAL LEFT OUTER JOIN customer_flights
    GROUP BY customerid, airlineid, name, frequentflieron
)
SELECT  customerid, name
FROM    customer_joined AS S
WHERE   airlineid = frequentflieron
AND     count <= ALL (
    SELECT  count
    FROM    customer_joined AS T
