...
public class AddPerson extends HttpServlet {
    ...
    public void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        if (first.length() > 12 || last.length() > 18 || age_int < 18 ||
                major.length() > 20 || relationship_int > 3 ||
                relationship_int < 1 || language.length() > 3 ||
                county.length() > 20) {
            ...
            int id = Person.addPerson(first, last, age_int, major, gender_int,
                    relationship_int, seeking_int, language, county, 1.0, foo);

            // Sends the response to the add page so that another person can be
            // added. ID is passed as a parameter to display the id for the new
            // user to refer to get and view matches
